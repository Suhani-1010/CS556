...
def simulate_voq_arrival_priority(packets):
    time = 0
    served = 0
    ...
    while served < total_packets:
        ...
        for pkt in remaining[:]:
            if pkt[1] == time:
                ...
                print(f"  Arrival: {pkt[0]} added to I{pkt[2]}->O{pkt[3]}")
                remaining.remove(pkt)

        # Collect head packets
        candidates = []
        for i in range(3):
            for o in range(3):
                if queues[i][o]:
                    candidates.append(queues[i][o][0])
        ...
        if not sent_packets and not remaining:
...
simulate_voq_arrival_priority(packets)